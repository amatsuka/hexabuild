...
namespace Game.Storage
{
    ...
    public sealed class ResourceIconBaker
    {
        ...
        void EnsureStage()
        {
            ...
            // Амбиент сцены живёт сферическими гармониками, и считает их движок сам — но только
            // в кадре. Снимки же пекутся до первого кадра, из `Bind`, и в плеере `SampleSH`
            // в шейдере читает тогда ещё не заполненный `unity_SH*`: мусор с порядками вроде
            // 1e31. Поле этого не замечает — оно рисуется каждый кадр и к первому же готовому
            // амбиенту приходит верным, а снимок печётся один раз и запоминает мусор навсегда.
            // Ровно отсюда кислотные иконки в вебе; в редакторе амбиент посчитан заранее.
            ...
            // ни постобработки, ни теней, ни копий глубины и цвета — на один меш в пустоте
            // это лишние проходы. Модель освещена ключевым светом сцены, и этого хватает.
